...
설계는 기획 정의가 충분할 때만 작성한다.
요구사항을 유추해서 API, DB, UI 흐름을 확정하지 않는다.

설계 전 반드시 확인:
- 목표와 비목표가 구분되어 있는가
- 사용자 흐름과 상태 전이가 명확한가
- 데이터 저장 위치와 API 계약이 정해졌는가
- 권한/예외/빈 상태/중복 처리 기준이 있는가
- acceptance criteria가 테스트 가능한 문장으로 존재하는가

불명확한 항목이 있으면 design.md에 "기획 미정/확인 필요"로 남기고, 구현 단계로 넘기지 않는다.
