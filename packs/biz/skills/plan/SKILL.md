...
## ⚠ 분석 원칙
- 일정은 **예상의 1.5~2배**를 현실적 기간으로 제시한다 (개발자 낙관 편향 보정)
- "이것도 하고 저것도 하고"가 아니라 **버릴 것을 명확히** 한다
- Kill criteria(중단 기준)를 구체적 숫자로 정의한다 — "안 되면 접는다"가 아니라 "3개월 내 유료 전환 50명 미달 시 중단"
...
- 산출물은 `workspace/current/plan.md`에 저장한다. 산출물 구조·frontmatter는 `.ace/templates/plan.template.md`를 따른다.
...
- **에이전트 위임**: 전체 섹션을 한번에 작성해야 하는 대규모 작업만 (위임 대상: `planner` 서브에이전트)
